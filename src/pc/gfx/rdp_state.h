...
typedef struct {
    ...
    LoadedTexture  loaded[GFX_SHADER_TEXTURES]; // TEXEL0/TEXEL1 tiles
    LoadedTexture  loaded_tiles[GFX_RDP_TILE_COUNT];
    TmemTextureLoad tmem_loads[GFX_RDP_TILE_COUNT];

    TileDesc  tile[GFX_RDP_TILE_COUNT];
    const u8* tlut;                // 256-colour RGBA5551 palette, set by gDPLoadTLUT
    const u8* tlut_pal[16];        // 16-colour RGBA5551 palette banks for CI4
    bool      tile_dirty[GFX_RDP_TILE_COUNT]; // load, TLUT, or descriptor changed
    u8        bound_texture_tile[GFX_SHADER_TEXTURES];
    u8        active_texture_tile; // selected render tile
