...
#include <stdint.h>
#define _GBI_ADDR(x) (uintptr_t)(x)
...
{									\
	...
	_g->words.w1 = _GBI_ADDR(s);				\
}

#define	gsDma0p(c, s, l)						\
{{									\
	_SHIFTL((c), 24, 8) | _SHIFTL((l), 0, 24), _GBI_ADDR(s)	\
}}
...
{									\
	Gfx *_g = (Gfx *)(pkt);						\
									\
	_g->words.w0 = _SHIFTL((c), 24, 8);				\
	_g->words.w1 = _GBI_ADDR(p0);				\
}

#define	gsImmp1(c, p0)							\
{{									\
	_SHIFTL((c), 24, 8), _GBI_ADDR(p0)				\
}}
...
{									\
	...
	_g->words.w0 = _SHIFTL(G_CULLDL, 24, 8) |			\
                       ((0x0f & (vstart))*40);				\
	_g->words.w1 = _GBI_ADDR((0x0f & ((vend)+1))*40);		\
}
...
{									\
	...
	_g->words.w1 = _GBI_ADDR(word);				\
}

#define	gsSPSetGeometryMode(word)					\
{{									\
	_SHIFTL(G_SETGEOMETRYMODE, 24, 8), _GBI_ADDR(word)		\
}}
...
{{									\
	_SHIFTL(G_CLEARGEOMETRYMODE, 24, 8), _GBI_ADDR(word)	\
}}
#endif	/* F3DEX_GBI_2 */
...
{									\
	...
	_g->words.w0 =	_SHIFTL(G_SETCOMBINE, 24, 8) |			\
			_SHIFTL(GCCc0w0(G_CCMUX_##a0, G_CCMUX_##c0,	\
			       ...
			       0, 24);					\
	_g->words.w1 =	_GBI_ADDR(GCCc0w1(G_CCMUX_##b0, 		\
					       G_CCMUX_##d0,		\
					       G_ACMUX_##Ab0, 		\
					       G_ACMUX_##Ad0) |		\
				       GCCc1w1(G_CCMUX_##b1, 		\
...
{{									\
	...
	_GBI_ADDR(GCCc0w1(G_CCMUX_##b0, G_CCMUX_##d0,		\
...
{									\
	Gfx *_g = (Gfx *)(pkt);						\
									\
	_g->words.w0 = _SHIFTL(c, 24, 8);				\
	_g->words.w1 = _GBI_ADDR(d);				\
}

#define	gsDPSetColor(c, d)						\
{{									\
	_SHIFTL(c, 24, 8), _GBI_ADDR(d)				\
}}
...
#define gsDPWord(wordhi, wordlo)			\
    gsImmp1(G_RDPHALF_1, _GBI_ADDR(wordhi)),	\
    gsImmp1(G_RDPHALF_2, _GBI_ADDR(wordlo))

#define gDPWord(pkt, wordhi, wordlo)      		\
{							\
    Gfx *_g = (Gfx *)(pkt);				\
							\
    gImmp1(pkt, G_RDPHALF_1, _GBI_ADDR(wordhi));	\
    gImmp1(pkt, G_RDPHALF_2, _GBI_ADDR(wordlo));	\
}
